...
@objc public class Adapty: NSObject {
    ...
    @objc public class func makePurchase(product: ProductModel, applicationUsername: String? = nil, offerId: String? = nil, completion: @escaping BuyProductCompletion) {
        LoggerManager.logMessage("Calling now: \(#function)")
        ...
        shared.iapManager.makePurchase(product: product, applicationUsername: applicationUsername, offerId: offerId) { (purchaserInfo, receipt, appleValidationResult, product, error) in
            if let error = error {
